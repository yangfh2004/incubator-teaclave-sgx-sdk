...
fn validate_json_web_token(
    ...
    validation.iss = Some("Intel Trust Authority".to_string());
...
fn main() {
    ...
    let res = client
        ...
        .header(
            "X-API-KEY",
            std::env::var("ITA_API_KEY").unwrap_or_else(|_| ITA_API_KEY.to_string()),
        )
        .json(&attest_request)
        .send()
        .unwrap();
    ...
    let res = client.get(ITA_CERTS_URL).send().unwrap();
    ...
    let claims = validate_json_web_token(jwt, jwk_set, jwt::Algorithm::RS256).unwrap();
    println!(
    ...
    println!(
        ...
        std::str::from_utf8(&base64::decode(&claims.x_ms_sgx_ehd).unwrap()).unwrap()
    );
}
